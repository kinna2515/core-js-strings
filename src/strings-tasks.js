...
function findLongestWord(sentence) {
  const longWord = sentence.split(' ').sort(function getLongWord(s, l) {
...
function reverseWords(str) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (let j = 0; j < words.length; j += 1) {
      lines[i] = words[j].split('').reverse().join('');
    }
    wordsRever = lines.join(' ');
  }
...
function getStringFromTemplate(firstName, lastName) {
  const greetList = ['Hello,', ' ', firstName, ' ', lastName, '!'];
  return ''.concat(...greetList);
}
...
function extractNameFromTemplate(value) {
  return value.slice(7, -1).toString();
}
...
function encodeToRot13(str) {
  ...
  const abcRot = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let changeStr = '';

  for (let i = 0; i < str.length; i += 1) {
    const letterCode = str[i];
    const abcCodeSymbol = abc.indexOf(letterCode);
    if (abcCodeSymbol === -1) {
      changeStr += letterCode;
    } else {
      const rotCodeSymbol = abcRot[abcCodeSymbol];
      changeStr += rotCodeSymbol;
    }
  }
  return changeStr;
}
